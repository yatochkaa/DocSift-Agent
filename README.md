...
Приложение рассчитано на локальный однопользовательский запуск на `127.0.0.1`.
Аутентификация, авторизация и изоляция арендаторов отсутствуют.
Выставлять порты 8000 (веб-интерфейс) и 5432 (Postgres) в сеть нельзя —
в Docker-контейнере они привязаны к `127.0.0.1`.
